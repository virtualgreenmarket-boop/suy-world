...
const DOG_CONFIGS = [
  ...
  { color: 0x2E2319, x: -80, z:  35 },  // dark brown/black
];
...
export function initDogs(scene) {
  DOG_CONFIGS.forEach(cfg => {
    ...
    dog.group.position.set(cfg.x, 0, cfg.z);
...
function updateDog(dog, delta, time) {
  // Tail wag (faster and bigger arc than cats)
  ...
  dog.group.position.y = Math.abs(bob) * 0.4;
}
...
function buildDog(color) {
  ...
  const bodyGeo = new THREE.CapsuleGeometry(0.19, 0.38, 4, 8);
  ...
  // Head (wider, more boxy feel via scale)
  ...
  // Floppy ears (flat boxes angled down from head)
  ...
  // Tail (upright, short — wagged via rotation.y)
  const tailGeo = new THREE.CylinderGeometry(0.028, 0.05, 0.24, 6);
