...
const CAT_CONFIGS = [
  ...
  { color: 0xBF360C, x:  90, z:  45 },  // tabby orange
];
...
export function initCats(scene) {
  CAT_CONFIGS.forEach(cfg => {
    ...
    cat.group.position.set(cfg.x, getSurfaceY(cfg.x, cfg.z), cfg.z);
...
function updateCat(cat, delta, time) {
  ...
  // Keep cat on the terrain surface
...
function startWalking(cat) {
  ...
  do {
    ...
    tries++;
    // Keep cats on normal terrain, away from plaza
...
function buildCat(color) {
  ...
  const eyeMat  = new THREE.MeshStandardMaterial({
    color: 0xFFFF00, emissive: 0xCCCC00, emissiveIntensity: 0.6, roughness: 0.3,
  });
  const noseMat = new THREE.MeshStandardMaterial({ color: 0xFF8A80, roughness: 0.9 });

  const group = new THREE.Group();

  const body = new THREE.Mesh(new THREE.CapsuleGeometry(0.13, 0.28, 4, 8), bodyMat);
  ...
  const tail = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.05, 0.32, 6), bodyMat);
